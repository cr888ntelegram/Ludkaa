...
SESSION_PATH = os.environ.get("SESSION_PATH", "gift_session.session")  # путь к сессии
# =====================================================
...
async def send_gift(username: str, gift_id: int, gift_name: str = "подарок") -> bool:
    try:
        ...
        # Используем InputInvoiceStarGift
        invoice = types.InputInvoiceStarGift(
            peer=peer,
            gift_id=gift_id,
            hide_name=False,
            message=types.TextWithEntities(text=GIFT_COMMENT, entities=[])
        )
...
async def main():
    ...
    # Подключаем Telethon через файл сессии
    try:
        log.info(f"📱 Подключаем Telethon через сессию: {SESSION_PATH}")
